...
import { BASE_URL_VIDEO, TEMP_WORKOUT_ID, VIDEO_CUSTOM_ID_PREFIX } from '../../../utils/constants';
...
const ApiChartView = ({ item, store }) => {
     ...
     */
     ...
     const getSensorDataById = async (sensorId) => {
        const res = await fetch(`${BASE_URL_VIDEO}get_workout_sensor_data_by_id/${sensorId}`);
    ...
    const fetchDataFromApi = async () => {
        ...
        setErrMsg('');
        try {
            const res = await fetch(`${URL}${WORKOUT_ID}`);
            const { ok, status } = res;
            if (ok && (status >= 200 && status <= 400)) {
                const response = await res.json();
                const { workout_detail_ids } = response;
